...
inline T2
endian_swap(const T& v) noexcept
{
  return v;
}
template<>
inline uint16_t
endian_swap<uint16_t, uint16_t>(const uint16_t& v) noexcept
{
  return (v << 8) | (v >> 8);
}
template<>
inline uint32_t
endian_swap<uint32_t, uint32_t>(const uint32_t& v) noexcept
{
...
template<>
inline uint64_t
endian_swap<uint64_t, uint64_t>(const uint64_t& v) noexcept
{
  return (
...
template<>
inline int16_t
endian_swap<int16_t, int16_t>(const int16_t& v) noexcept
{
...
template<>
inline int32_t
endian_swap<int32_t, int32_t>(const int32_t& v) noexcept
{
...
template<>
inline int64_t
endian_swap<int64_t, int64_t>(const int64_t& v) noexcept
{
...
template<>
inline float
endian_swap<uint32_t, float>(const uint32_t& v) noexcept
{
  union
  {
    float f;
...
template<>
inline double
endian_swap<uint64_t, double>(const uint64_t& v) noexcept
{
  union
  {
    double d;
...
inline uint32_t
hash_fnv1a(const std::string& str) noexcept
{
...
inline Type
property_type_from_string(const std::string& t) noexcept
{
  if (t == "int8" || t == "char")
    return Type::INT8;
...
struct PlyFile::PlyFileImpl
{
  ...
  size_t read_property_binary(const size_t& stride,
                              void* dest,
                              size_t& destOffset,
                              std::istream& is) noexcept;
  size_t read_property_ascii(const Type& t,
                             const size_t& stride,
                             void* dest,
                             size_t& destOffset,
                             std::istream& is);
  ...
  void read_header_text(std::string line,
  ...
  void write_property_ascii(Type t,
                            std::ostream& os,
                            uint8_t* src,
                            size_t& srcOffset);
  void write_property_binary(std::ostream& os,
                             uint8_t* src,
                             size_t& srcOffset,
...
template<typename T, typename T2>
inline void
endian_swap_buffer(uint8_t* data_ptr,
...
template<typename T>
void
ply_cast_ascii(void* dest, std::istream& is)
{
...
bool
PlyFile::PlyFileImpl::parse_header(std::istream& is)
{
  std::string line;
  bool success = true;
...
void
PlyFile::PlyFileImpl::read_header_text(std::string line,
                                       std::vector<std::string>& place,
                                       int erase)
{
...
void
PlyFile::PlyFileImpl::read_header_format(std::istream& is)
{
  std::string s;
  (is >> s);
...
void
PlyFile::PlyFileImpl::read_header_element(std::istream& is)
{
  elements.emplace_back(is);
}

void
PlyFile::PlyFileImpl::read_header_property(std::istream& is)
{
  if (!elements.size())
...
size_t
PlyFile::PlyFileImpl::read_property_binary(const size_t& stride,
                                           void* dest,
                                           size_t& destOffset,
                                           std::istream& is) noexcept
{
...
size_t
PlyFile::PlyFileImpl::read_property_ascii(const Type& t,
                                          const size_t& stride,
                                          void* dest,
                                          size_t& destOffset,
                                          std::istream& is)
...
void
PlyFile::PlyFileImpl::write_property_ascii(Type t,
                                           std::ostream& os,
                                           uint8_t* src,
                                           size_t& srcOffset)
{
...
void
PlyFile::PlyFileImpl::write_property_binary(std::ostream& os,
                                            uint8_t* src,
                                            size_t& srcOffset,
                                            const size_t& stride) noexcept
{
...
void
PlyFile::PlyFileImpl::read(std::istream& is)
{
  ...
  if (isBigEndian) {
    for (auto& b : buffers) {
      ...
      switch (b->t) {
        case Type::INT16:
          endian_swap_buffer<int16_t, int16_t>(
            data_ptr, buffer_size_bytes, stride);
          break;
        case Type::UINT16:
          endian_swap_buffer<uint16_t, uint16_t>(
            data_ptr, buffer_size_bytes, stride);
          break;
        case Type::INT32:
          endian_swap_buffer<int32_t, int32_t>(
            data_ptr, buffer_size_bytes, stride);
          break;
        case Type::UINT32:
          endian_swap_buffer<uint32_t, uint32_t>(
            data_ptr, buffer_size_bytes, stride);
          break;
        case Type::FLOAT32:
          endian_swap_buffer<uint32_t, float>(
            data_ptr, buffer_size_bytes, stride);
          break;
        case Type::FLOAT64:
          endian_swap_buffer<uint64_t, double>(
            data_ptr, buffer_size_bytes, stride);
          break;
        default:
          break;
...
void
PlyFile::PlyFileImpl::write(std::ostream& os, bool _isBinary)
{
  for (auto& d : userData) {
...
void
PlyFile::PlyFileImpl::write_binary_internal(std::ostream& os) noexcept
{
  isBinary = true;
...
void
PlyFile::PlyFileImpl::write_ascii_internal(std::ostream& os) noexcept
{
  write_header(os);
...
void
PlyFile::PlyFileImpl::write_header(std::ostream& os) noexcept
{
  ...
  const uint32_t list_size_hint)
{
  ...
  if (elementIndex >= 0) {
    ...
    if (std::adjacent_find(propertyTypes.begin(),
                           propertyTypes.end(),
...
void
PlyFile::PlyFileImpl::add_properties_to_element(
  const std::string& elementKey,
  const std::vector<std::string> propertyKeys,
  const Type type,
  const size_t count,
...
void
PlyFile::PlyFileImpl::parse_data(std::istream& is, bool firstPass)
{
  std::function<void(PropertyLookup & f,
    ...
    read;
  std::function<size_t(
    PropertyLookup & f, const PlyProperty& p, std::istream& is)>
    skip;
  ...
  if (isBinary) {
    read = [this, &listSize, &dummyCount, &read_list_binary](
      ...
      if (!p.isList) {
        return read_property_binary(
          f.prop_stride, dest + destOffset, destOffset, _is);
      }
    ...
    read = [this, &listSize, &dummyCount](PropertyLookup& f,
      ...
      if (!p.isList) {
        read_property_ascii(
          p.propertyType, f.prop_stride, dest + destOffset, destOffset, _is);
      } else {
    ...
    skip =
      [this, &listSize, &dummyCount, &skip_ascii_buffer](
...
PlyFile::~PlyFile() {}
bool
PlyFile::parse_header(std::istream& is)
{
  return impl->parse_header(is);
}
void
PlyFile::read(std::istream& is)
{
  return impl->read(is);
}
void
...
PlyFile::get_elements() const
{
  return impl->elements;
}
std::vector<std::string>&
PlyFile::get_comments()
{
  return impl->comments;
}
std::vector<std::string>
PlyFile::get_info() const
{
  return impl->objInfo;
}
bool
PlyFile::is_binary_file() const
{
...
PlyFile::request_properties_from_element(
  ...
  const uint32_t list_size_hint)
{
  return impl->request_properties_from_element(
    elementKey, propertyKeys, list_size_hint);
}
void
PlyFile::add_properties_to_element(const std::string& elementKey,
                                   const std::vector<std::string> propertyKeys,
                                   const Type type,
                                   const size_t count,
                                   uint8_t* data,
